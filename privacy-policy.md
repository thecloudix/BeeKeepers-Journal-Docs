...
3. **Open-Meteo weather lookup.** When you tap "Use current weather" on the inspection form, the app sends your device's latitude and longitude to `api.open-meteo.com` (a free, non-commercial weather service) to fetch the current temperature and a short conditions string. No hive data, inspection data, account identifier, or device identifier is sent — only the two coordinates and a generic `User-Agent` header naming the app. Open-Meteo is governed by its own [privacy policy](https://open-meteo.com/en/privacy-policy). If you never tap that button, this request is never made.
...
| 2026-05-13 | Added optional weather lookup on the inspection form. When you tap "Use current weather", the app sends only your device's latitude and longitude to `api.open-meteo.com` to retrieve the current temperature and conditions. No hive data is included. The feature does not run unless you tap the button. |
